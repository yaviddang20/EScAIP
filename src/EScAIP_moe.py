...
@registry.register_model("EScAIP_backbone_moe")
class EScAIPBackboneMoE(nn.Module, GraphModelMixin):
    """
    ...
    def compiled_forward(self, data: GraphAttentionData):
        ...
        for idx in range(self.gnn_cfg.num_layers):
            node_features, edge_features, load_loss = self.transformer_blocks[idx](
...
class EScAIPHeadBase(nn.Module, HeadInterface):
    def __init__(self, backbone: EScAIPBackboneMoE):
        super().__init__()
...
@registry.register_model("EScAIP_load_loss_head")
class EScAIPLoadLossHead(EScAIPHeadBase):
    def __init__(self, backbone: EScAIPBackboneMoE):
        super().__init__(backbone)
        self.post_init()

    def compiled_forward(self, load_loss):
        return load_loss

    def forward(self, data, emb: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        return {"load_loss": self.forward_fn(load_loss=emb["load_loss"])}
